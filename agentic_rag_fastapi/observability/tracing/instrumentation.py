...
def wrapped_safe_generate(prompt: str, temperature: float = 0):
    ...
    try:
        ...
        error_type = "Rate Limit Error" if "rate" in str(e).lower() or "429" in str(e) else "LLM Error"
...
def wrapped_query_rewriter(sub_query: str):
    ...
    rewrite_reason = "Planner subquery optimization" if iteration == 0 else "Feedback loop query enhancement"
...
def setup_observability():
    ...
    for mod_name, mod in list(sys.modules.items()):
        if mod is None or mod_name == __name__:
            continue
        try:
            if hasattr(mod, "agentic_rag") and getattr(mod, "agentic_rag") is not wrapped_agentic_rag:
                setattr(mod, "agentic_rag", wrapped_agentic_rag)
            if hasattr(mod, "vanilla_rag") and getattr(mod, "vanilla_rag") is not wrapped_vanilla_rag:
                setattr(mod, "vanilla_rag", wrapped_vanilla_rag)
            if hasattr(mod, "planner_agent") and getattr(mod, "planner_agent") is not wrapped_planner_agent:
                setattr(mod, "planner_agent", wrapped_planner_agent)
            if hasattr(mod, "query_rewriter") and getattr(mod, "query_rewriter") is not wrapped_query_rewriter:
            ...
            if hasattr(mod, "sufficient_context_agent") and getattr(mod, "sufficient_context_agent") is not wrapped_sufficient_context_agent:
                setattr(mod, "sufficient_context_agent", wrapped_sufficient_context_agent)
            if hasattr(mod, "synthesis_agent") and getattr(mod, "synthesis_agent") is not wrapped_synthesis_agent:
                setattr(mod, "synthesis_agent", wrapped_synthesis_agent)
            if hasattr(mod, "safe_generate") and getattr(mod, "safe_generate") is not wrapped_safe_generate:
