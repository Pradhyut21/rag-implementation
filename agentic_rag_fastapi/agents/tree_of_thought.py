...
def evaluate_branch(branch: Dict[str, Any], query: str, embedding_model, vector_store, top_k: int = 3) -> Dict[str, Any]:
    """
...
"""
    ...
    retrieval_similarity = max(0.0, min(1.0, 1.0 - (avg_similarity / 2.0))) if avg_similarity > 0 else 0.0
...
def select_best_branch(branches: List[Dict[str, Any]]) -> Tuple[Dict[str, Any], List[Dict[str, Any]]]:
    """
...
def merge_branches(branches: List[Dict[str, Any]], score_threshold: float = 0.6, margin: float = 0.15) -> Tuple[List[str], bool]:
    """
    ...
    if top_score >= score_threshold and (top_score - second_score) <= margin:
        logger.info(f"Merging Branch A ({top_branch['branch_name']}) and Branch B ({second_branch['branch_name']})")
